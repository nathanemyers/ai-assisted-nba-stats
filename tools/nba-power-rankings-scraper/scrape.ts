...
function articleFilePath(
  article: Pick<PowerRankingsArticle, 'season' | 'slug'>
): string {
...
export async function scrape(options: ScrapeOptions): Promise<void> {
  ...
  const listItems = await listAllPowerRankingsArticles({
  ...
  for (const [i, item] of inRange.entries()) {
    ...
    try {
      ...
      const existingPath = options.force
        ? null
        : await findExistingArticleFile(options.outDir, item.slug)

      if (existingPath) {
        article = JSON.parse(
          await readFile(existingPath, 'utf8')
        ) as PowerRankingsArticle
        skipped++
      } else {
...
async function findExistingArticleFile(
  outDir: string,
  slug: string
): Promise<string | null> {
